...
export default function AddBook() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/books", { 
        headers: {
          Authorization: `Bearer`,//get acsess token
        },
      })
      .then((resp) => {
  ...
  return (
    ...
    {bookData.err && (
      ...
      {/*  title  */}
      <div class="p-2">
        <input type="text" id="title" name="title" placeholder="Book Title" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#8c0327] focus:ring-[#8c0327] focus:ring-opacity-50 p-2" style={{backgroundColor: "#f6f6f6"}} 
      ...
      <div class="p-2 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <textarea id="description" name="description" rows="3" placeholder="Book Description" class="block w-full h-48 rounded-md border-gray-300 shadow-sm focus:border-[#8c0327] focus:ring-[#8c0327] focus:ring-opacity-50 p-2" style={{backgroundColor: "#f6f6f6"}}
            onChange={handleChange}
      ...
      {/*  Book Img Upload  */}
        <div>
          <label for="img" class="block w-full h-48 border-2 border-dashed border-gray-300 rounded-md cursor-pointer flex flex-col items-center justify-center bg-[#f6f6f6] hover:bg-gray-50">
          ...
          <input id="img" name="img" type="file" accept="image/*" class="sr-only" ref={img}/>
        </div>
      </div>
  
      <div class="p-2 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <input type="text" id="author" name="author" placeholder="Author Name" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#8c0327] focus:ring-[#8c0327] focus:ring-opacity-50 p-2" style={{backgroundColor: "#f6f6f6"}} 
          onChange={handleChange}
